...
const ProfileCard = () => {
  ...
  const { fetchMlmData } = AllTempDataProvider();
  ...
  const logOut = async () => {
    ...
    // AsyncStorage?.setItem("profileImage", JSON.stringify({}));
    await AsyncStorage?.setItem("bname", JSON.stringify({}));
    setDATAMLMUser({});
    setMlmTemp([]);
    setUserData({});
    setTimeout(() => {
    ...
    <View style={[{ width: "100%", height: "100%", backgroundColor: "#fff" }]}>
      ...
      >
        ...
        >
          ...
          {/* <Pressable
            onPress={() =>
              Linking.openURL("https://wa.me/message/KPSZTV4SZ5RNP1")
            }
            style={tw`flex bg-[${bgcolor}] flex-row gap-4 w-full justify-start items-center  rounded-lg p-4 `}
          >
            <Icon as={<StarIcon color={color} size={25} />} />
            <Text allowFontScaling={false} style={[tw`text-[16px] font-bold text-black`]}>
              Feedback & Review
            </Text>
          </Pressable> */}
          ...
          >
            ...
            <Text allowFontScaling={false} style={tw`text-[${color}] font-semibold`}>
